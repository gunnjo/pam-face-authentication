...
//------------------------------------------------------------------------------
tracker::tracker() : trackerModelFeatureSizeX(0), trackerModelFeatureSizeY(0),
  trackerModelFeatureVARIANCEX(0), trackerModelFeatureVARIANCEY(0),
...
void tracker::trackImage(IplImage* input)
{
  ...
  double v1 = runGridSearch(input, trackerModelFeatureSizeY, 1,
    &stateVariableScaleY, &stateVariableTranslateY, trackerModelFeatureVARIANCEY,
    trackerModelFeatureINTEGRALY, anchorPoint.y);
  double v2 = runGridSearch(input, trackerModelFeatureSizeX, 0,
    &stateVariableScaleX, &stateVariableTranslateX, trackerModelFeatureVARIANCEX,
    trackerModelFeatureINTEGRALX,anchorPoint.x);
...
{
  (*p2).x = floor( (double)(stateVariableScaleX * (double)p1.x
      + (double)(stateVariableTranslateX))
    * ((double)(lastImageWidth) / (double)(trackerModelFeatureSizeX)));
  (*p2).y = floor( (double)(stateVariableScaleY * (double)p1.y
      + (double)(stateVariableTranslateY))
    * ((double)(lastImageHeight) / (double)(trackerModelFeatureSizeY)));
}
...
double* tracker::calculateFeature(IplImage* input, int flag, int varorintegral)
{
  ...
  for(int i = 0; i < lim1; i++) integral[i] = 0;
  for(int i = 0; i < lim1; i++)
  {
    for(int j = 0; j < lim2; j++)
    {
      CvScalar s;
      if(flag == 0) s = cvGet2D(input, j, i);
        else s = cvGet2D(input, i, j);
  ...
  if(flag == 0) var = new double[input->width];
    else var = new double[input->height];
  ...
  for(int i = 0; i < lim1; i++)
  {
    ...
    /*if(i>0)
    {
      if(pow((var[i] - var[i-1]), 2) <1) var[i] = floor(var[i]);
    }
    printf("%e %e \n",integral[i],var[i]);
    */
  }
...
//------------------------------------------------------------------------------
double tracker::findParam(double scaleFactor, double translateFactor,
  double* updatedScaleFactor, double* updatedTranslateFactor, double* feature,
  double* feature1, double* featureModel, double* featureModel1,
  int size, double scale, double translate, int anchor)
{
  double val, px, py;
...
//------------------------------------------------------------------------------
double tracker::difference(double* feature, double* featureModel, int size,
  double px, double py, int anchor)
{
  ...
  for(i = 0; i < size; i++)
  {
    if((px*i +py) >= 0 && (px*i +py) < (size-1))
    {
      ...
      if( (i > int(anchor - floor(.07*size)))
      && ( i < int(anchor+ floor(.07*size))))
      {
        // printf("true anchor \n",k);
